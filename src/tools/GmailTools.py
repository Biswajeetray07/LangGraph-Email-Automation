...
class GmailToolsClass:
    ...
    def fetch_recent_emails(self, max_results=50):
        try:
            # Get lookback hours from environment or default to 8
            lookback_hours = int(os.getenv("EMAIL_LOOKBACK_HOURS", 8))
            now = datetime.now()
            delay = now - timedelta(hours=lookback_hours)
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                if not os.path.exists(credentials_path):
                    raise FileNotFoundError(f"Missing '{credentials_path}'. Please follow the setup guide to obtain it.")
