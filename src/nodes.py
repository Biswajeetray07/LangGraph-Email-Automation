...
class Nodes:
    ...
    def categorize_email(self, state: GraphState) -> GraphState:
        ...
        if not state.get("emails"):
            print(Fore.RED + "Error: No emails to categorize." + Style.RESET_ALL)
            return state
        ...
        print(Fore.MAGENTA + f"Email category identified: {result.category.value}" + Style.RESET_ALL)
    ...
    def must_rewrite(self, state: GraphState) -> str:
        ...
        if email_sendable:
            ...
            print(Fore.RED + "Email rewrite limit reached. Stopping processing for this email." + Style.RESET_ALL)
            if state.get("emails"):
                state["emails"].pop()
            state["writer_messages"] = []
            return "stop"
